...
GPU_PIPELINE_VERSION = "2026-07-14-GPU-V2"
print("=" * 80)
...
if not hasattr(ort, "preload_dlls"):
    raise RuntimeError(
        "当前 onnxruntime-gpu 不支持 preload_dlls；"
        f"当前版本：{getattr(ort, '__version__', 'unknown')}"
    )
...
try:
    ...
    raise RuntimeError(
        f"CUDA/cuDNN/MSVC DLL 预加载失败：{exc}"
    ) from exc

_AVAILABLE_PROVIDERS = list(ort.get_available_providers())

print(
    "[CUDA] ONNX Runtime DLL 已预加载；Provider："
...
try:
    ...
    raise RuntimeError(
        "没有安装 PyTorch CUDA。请先运行 install_road_gpu_acceleration_fixed.bat。"
        f"\n原始错误：{exc}"
    ) from exc

if not torch.cuda.is_available():
    raise RuntimeError(
        "PyTorch 已安装，但 torch.cuda.is_available() 为 False。"
