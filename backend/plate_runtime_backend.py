...
2. 进程/线程解耦：主进程运行视频与 ONNX CUDA；独立子进程运行 PaddleOCR GPU。
...
5. OCR 默认使用 PP-OCRv6_tiny_rec GPU 独立子进程，彻底隔离 CUDA/cuDNN DLL。
...
try:
    from .runtime_road_anomaly_detector import RuntimeRoadAnomalyDetector
except ImportError:
    from runtime_road_anomaly_detector import RuntimeRoadAnomalyDetector
...
_CUDA_PRELOAD_INFO: Dict[str, Any] = {"ok": False, "message": "not started", "dll_dirs": []}

# 模型切换和前端轮询可能在短时间内重复触发 load_models。
# 串行化模型加载，避免 ONNX Session 和 PaddleX/OCR 被并发初始化。
_MODEL_LOAD_LOCK = threading.Lock()
_OCR_LOAD_LOCK = threading.Lock()
_OCR_LOAD_ATTEMPTED = False
_OCR_LOAD_ERROR = ""
BACKEND_FIX_VERSION = "2026-07-14-OCR-GPU-SUBPROCESS-V6"


def preload_cuda_runtime_for_onnx() -> Dict[str, Any]:
    """按正确顺序准备 Windows CUDA/cuDNN DLL。

    关键顺序：
    1. 优先导入并初始化 PyTorch CUDA，让 torch/lib 中与 PyTorch 匹配的
       CUDA/cuDNN DLL 先进入当前进程。
    2. 再让 ONNX Runtime 使用 directory=None 复用这一套 DLL。
    3. 只有系统没有可用 PyTorch CUDA 时，才回退到 site-packages/nvidia。

    这样可避免先加载 nvidia 包中的 cuDNN 后，PaddleOCR -> ModelScope
    再导入 torch 时出现 cudnn_cnn64_9.dll / WinError 127。
    """
    global _CUDA_DLL_PRELOADED, _CUDA_PRELOAD_INFO

    if _CUDA_DLL_PRELOADED:
        return dict(_CUDA_PRELOAD_INFO)
    ...
    try:
        # 先让 PyTorch 加载它自己配套的 CUDA/cuDNN DLL。
        try:
            import torch
            ...
            if bool(torch.cuda.is_available()):
                ...
                info["torch_message"] = (
                    f"torch={getattr(torch, '__version__', 'unknown')}，但 CUDA 不可用"
                )
        except Exception as torch_exc:
            info["torch_message"] = f"PyTorch CUDA 预加载跳过：{repr(torch_exc)}"

        # 仅用于状态展示和无 PyTorch 时的回退。
        ...
        if hasattr(ort, "preload_dlls"):
            if torch_ready:
                # 默认搜索顺序会优先使用已安装的 PyTorch torch/lib。
                ort.preload_dlls(
                    cuda=True,
                    cudnn=True,
                    msvc=True,
                ...
                # 没有可用 PyTorch 时，才把 pip NVIDIA 运行库加入搜索路径。
                if os.name == "nt":
        ...
        info["ok"] = True
        mode = "PyTorch torch/lib" if torch_ready else "site-packages/nvidia"
        info["message"] = (
            f"CUDA/cuDNN DLL preload finished via {mode}, "
            f"dll_dirs={len(dll_dirs)}"
        )
        info["dll_dirs"] = dll_dirs
        _CUDA_PRELOAD_INFO = dict(info)
        return info
    except Exception as exc:
        info["message"] = f"CUDA/cuDNN DLL preload failed: {repr(exc)}"
        info["dll_dirs"] = dll_dirs
        _CUDA_PRELOAD_INFO = dict(info)
        return info
...
class NormalRoiConfigureRequest(BaseModel):
    # 0~1 归一化四点道路 ROI。运行中配置时不会重新打开或重连视频源。
    points: List[List[float]] = []


class NormalBaselineRebuildRequest(BaseModel):
    # 重新从当前视频源采集正常道路基准，不重连视频源。
    force: bool = True
...
OCR_PROTOCOL_PREFIX = "@@TRAFFIC_OCR_JSON@@"
OCR_WORKER_PATH = ROOT_DIR / "backend" / "paddle_ocr_gpu_worker.py"
OCR_VENV_PYTHON = ROOT_DIR / ".venv_ocr" / "Scripts" / "python.exe"


class PaddleOCRSubprocessClient:
    """PaddleOCR GPU 独立进程客户端。

    主后端进程只加载 PyTorch/ONNX Runtime CUDA。
    OCR 子进程只加载 PaddlePaddle GPU。两套 cuDNN 位于不同 Windows
    进程地址空间，彻底避免 cudnn_cnn64_9.dll / WinError 127 冲突。
    """

    def __init__(self, model_name: str, device: str = "gpu:0") -> None:
        self.model_name = str(model_name or "PP-OCRv6_tiny_rec")
    ...
    def resolve_python() -> Path:
        configured = str(os.environ.get("TRAFFIC_OCR_PYTHON_EXE", "")).strip()
        candidates = []
        if configured:
            candidates.append(Path(configured))
        candidates.append(OCR_VENV_PYTHON)
        ...
        allow_main = str(
            os.environ.get("TRAFFIC_OCR_ALLOW_MAIN_VENV", "0")
    ...
    def is_ready(self) -> bool:
        return bool(self.ready and self.is_alive())
    ...
    def _stdout_reader(
        ...
        try:
            for raw_line in iter(stream.readline, ""):
                ...
                if not line:
                    continue
                if line.startswith(OCR_PROTOCOL_PREFIX):
                    payload_text = line[len(OCR_PROTOCOL_PREFIX):]
                    try:
                    ...
                    log(f"OCR subprocess stdout: {line}")
        except Exception as exc:
            message_queue.put({
                "type": "reader_error",
                "error": repr(exc),
    ...
    def _stderr_reader(self, process: subprocess.Popen) -> None:
        ...
        try:
            for raw_line in iter(stream.readline, ""):
                line = str(raw_line or "").strip()
                if line:
                    log(f"OCR subprocess stderr: {line}")
        except Exception as exc:
            log(f"OCR subprocess stderr reader stopped: {exc}")
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if not self.is_alive():
                        raise RuntimeError(
                            f"OCR 子进程已退出，returncode="
                            f"{self.process.poll() if self.process else None}"
                        )
                    continue
                ...
                if message_type == "process_exit":
                    raise RuntimeError(
                        f"OCR 子进程意外退出，returncode="
                        f"{message.get('returncode')}"
                    )

                id_matches = (
                    request_id is None
                    or int(message_id or -1) == int(request_id)
                )
                if message_type in expected_types and id_matches:
                    return message

                deferred.append(message)

            raise TimeoutError(
                f"等待 OCR 子进程响应超时：types={sorted(expected_types)}, "
    ...
    def start(self, timeout_s: float = 300.0) -> Dict[str, Any]:
        with self._lock:
            if self.is_ready():
            ...
            worker_python = self.resolve_python()
            if not OCR_WORKER_PATH.exists():
                raise FileNotFoundError(
                    f"缺少 OCR 子进程脚本：{OCR_WORKER_PATH}"
                )

            env = os.environ.copy()
            env["PYTHONUTF8"] = "1"
            env["PYTHONIOENCODING"] = "utf-8"
            env["PADDLE_PDX_MODEL_SOURCE"] = env.get(
                "PADDLE_PDX_MODEL_SOURCE",
                "BOS",
            )
            env["OMP_NUM_THREADS"] = "1"
            env["TRAFFIC_OCR_WORKER"] = "1"
            ...
            log(
                ...
                f"python={worker_python} model={self.model_name} "
            ...
            self._stdout_thread = threading.Thread(
                ...
                daemon=True,
                name="paddle-ocr-stdout",
            )
            self._stderr_thread = threading.Thread(
                target=self._stderr_reader,
                args=(process,),
                daemon=True,
                name="paddle-ocr-stderr",
            )
            self._stdout_thread.start()
            self._stderr_thread.start()

            message = self._wait_message_locked(
                {"ready"},
                timeout_s=timeout_s,
            )

            if not bool(message.get("ok")):
                error = str(message.get("error") or "OCR 子进程初始化失败")
    ...
    def recognize(
        ...
        with self._lock:
            ...
            if not ok:
                raise RuntimeError("OCR 车牌裁剪图编码失败。")
            ...
            if str(message.get("type")) == "error" or not bool(
                message.get("ok", True)
            ):
                error = str(message.get("error") or "OCR 识别失败")
                self.last_error = error
                raise RuntimeError(error)

            return (
                str(message.get("text") or ""),
                float(message.get("confidence") or 0.0),
                float(message.get("ocr_ms") or 0.0),
            )
...
def ocr_service_ready() -> bool:
    client = STATE.ocr_model
    return bool(
        isinstance(client, PaddleOCRSubprocessClient)
        and client.is_ready()
    )


def ensure_paddleocr_gpu_subprocess() -> bool:
    global _OCR_LOAD_ATTEMPTED, _OCR_LOAD_ERROR

    with _OCR_LOAD_LOCK:
        _OCR_LOAD_ATTEMPTED = True

        client = STATE.ocr_model
        if not isinstance(client, PaddleOCRSubprocessClient):
            client = PaddleOCRSubprocessClient(
                model_name=STATE.config.ocr_model_name,
                device="gpu:0",
            )
            STATE.ocr_model = client
        else:
            client.model_name = STATE.config.ocr_model_name

        try:
            metadata = client.start(timeout_s=300.0)
            _OCR_LOAD_ERROR = ""
            STATE.update_status(
                ocr_ready=True,
                ocr_model=STATE.config.ocr_model_name,
                ocr_device=str(metadata.get("device") or "gpu:0"),
                ocr_runtime=(
                    "PaddleOCR GPU 独立子进程 | "
                    f"paddle={metadata.get('paddle_version', 'unknown')} | "
                ...
                ocr_process_python=str(
                    metadata.get("python_executable")
                    or client.resolve_python()
                ),
                ocr_process_alive=True,
                ocr_error="",
            )
            return True
        except Exception as exc:
            _OCR_LOAD_ERROR = f"{type(exc).__name__}: {exc}"
            client.stop()
            STATE.update_status(
                ocr_ready=False,
                ocr_device="gpu:0",
                ocr_runtime="PaddleOCR GPU 独立子进程启动失败",
                ocr_process_pid=0,
                ocr_process_python=str(OCR_VENV_PYTHON),
                ocr_process_alive=False,
                ocr_error=_OCR_LOAD_ERROR,
            )
            log(f"PaddleOCR GPU subprocess start failed: {_OCR_LOAD_ERROR}")
            return False


def stop_paddleocr_gpu_subprocess() -> None:
    client = STATE.ocr_model
    if isinstance(client, PaddleOCRSubprocessClient):
        try:
            client.stop()
        except Exception as exc:
            log(f"Stopping OCR subprocess failed: {exc}")
    STATE.update_status(
        ocr_ready=False,
        ocr_process_pid=0,
        ocr_process_alive=False,
...
class RuntimeState:
    def __init__(self) -> None:
        ...
        self.status: Dict[str, Any] = {
            ...
            "ocr_device": "gpu:0",
            "ocr_runtime": "PaddleOCR GPU 独立子进程（等待启动）",
            "ocr_process_pid": 0,
            "ocr_process_python": "",
            "ocr_process_alive": False,
            ...
            "normal_baseline_status": "waiting_roi",
            "normal_baseline_count": 0,
            "normal_baseline_target": 12,
            "normal_baseline_progress": 0,
            "normal_anomaly_candidates": 0,
            "normal_anomaly_confirmed": 0,
            "normal_anomaly_total_events": 0,
            "normal_anomaly_ms": 0.0,
        }
        ...
        self.normal_anomaly_thread: Optional[threading.Thread] = None
        ...
        # 道路异常检测与车道线检测共用同一四点 ROI。
        # 基准必须从当前正在运行的视频源实时采集，绝不复用测试阶段的旧基准。
        self.normal_anomaly_detector = RuntimeRoadAnomalyDetector(
            ROOT_DIR / "runtime_data" / "road_anomaly"
        )
        self.normal_anomaly_result: Dict[str, Any] = (
            self.normal_anomaly_detector.empty_result(
                status="waiting_roi",
                message="请先选择四点道路 ROI。",
            )
        )
...
def _load_models_impl(*, allow_ocr: bool = True) -> None:
    ...
    STATE.update_status(
        ...
        message=(
            f"正在预加载 {model_info['display_label']} ONNX 检测模型"
            + ("与轻量 OCR 识别模型。" if bool(model_info["uses_ocr"]) else "；当前模式无需 OCR。")
        )
    )
    errors: List[str] = []
    ...
    # OCR 只属于车牌识别模式。PaddleOCR GPU 在独立 Python
    # 子进程中加载，主后端进程从不 import paddle / paddleocr。
    mode_requires_ocr = bool(model_info.get("uses_ocr", False))
    needs_ocr = bool(mode_requires_ocr and allow_ocr)

    if needs_ocr:
        if not ensure_paddleocr_gpu_subprocess():
            errors.append(f"OCR子进程加载失败: {_OCR_LOAD_ERROR}")
    else:
        STATE.update_status(
            ocr_ready=ocr_service_ready(),
            ocr_required=mode_requires_ocr,
            ocr_deferred=bool(
                mode_requires_ocr
                and not allow_ocr
                and not ocr_service_ready()
            ),
            ocr_process_alive=ocr_service_ready(),
            ocr_error=_OCR_LOAD_ERROR,
        )

    yolo_ready = bool(STATE.ort_session is not None)
    # models_ready 表示当前 ONNX 检测链路可运行。OCR 是车牌模式的附加能力，
    # 不能再阻塞本地视频、手机视频、道路异常和其他模式启动。
    ready = bool(yolo_ready)

    if ready:
        if mode_requires_ocr and ocr_service_ready():
            ready_message = f"{active_model_info()['display_label']} ONNX 与 OCR 模型预加载完成。"
        elif mode_requires_ocr and not allow_ocr:
            ready_message = f"{active_model_info()['display_label']} ONNX 已就绪；OCR 延迟到车牌工作流启动时加载。"
        elif mode_requires_ocr:
            ready_message = f"{active_model_info()['display_label']} ONNX 已就绪；OCR 当前不可用，仍可进行车牌区域检测。"
        else:
            ready_message = f"{active_model_info()['display_label']} ONNX 模型预加载完成；当前模式无需 OCR。"
    else:
        ready_message = "；".join(errors) or "ONNX 模型初始化未完成。"

    STATE.update_status(
        backend="ready" if ready else "degraded",
        models_ready=ready,
        model_uses_ocr=mode_requires_ocr,
        ocr_required=mode_requires_ocr,
        message=ready_message,
    )


# load_models 可能被 startup 与前端模型切换同时触发，必须串行执行。
def load_models(*, allow_ocr: bool = True) -> None:
    acquired = _MODEL_LOAD_LOCK.acquire(blocking=False)
    if not acquired:
        log("Model loading is already in progress; duplicate load request skipped.")
        return
    try:
        _load_models_impl(allow_ocr=allow_ocr)
    finally:
...
def recognize_plate(crop: np.ndarray) -> Tuple[str, float, float]:
    ...
    t0 = time.perf_counter()
    raw, conf, worker_ms = client.recognize(
        crop,
        timeout_s=10.0,
    )
    roundtrip_ms = (time.perf_counter() - t0) * 1000.0
    perf_add("ocr_recognize_ms", roundtrip_ms)
    perf_add("ocr_worker_gpu_ms", worker_ms)

    STATE.update_status(
        ocr_ready=client.is_ready(),
        ocr_process_alive=client.is_alive(),
        ocr_process_pid=int(
            client.process.pid
            if client.process is not None
            else 0
        ),
        ocr_worker_gpu_ms=round(worker_ms, 2),
        ocr_roundtrip_ms=round(roundtrip_ms, 2),
    )

    return PlateVoting.clean(raw), float(conf or 0.0), roundtrip_ms
    ...
    while not STATE.stop_event.is_set():
        ...
        try:
            ...
            STATE.update_status(
                ...
                ocr_process_alive=process_alive,
    ...
    while not STATE.stop_event.is_set():
        ...
        if source_type == "camera" and STATE.config.motion_reset_enabled:
            ...
            if motion_score >= STATE.config.motion_reset_score and now_motion - STATE.last_motion_reset_t >= STATE.config.motion_reset_min_interval:
                ...
                # normal 模式依赖固定机位实时基准。明显移动后旧基准已失效，
                # 因此保持同一视频连接并自动重新采集，而不是继续拿旧机位比较。
                if (
                    normalize_detector_model(getattr(STATE.config, "detector_model", "plate")) == "normal"
                    and len(STATE.normal_roi_normalized) == 4
                ):
                    STATE.normal_anomaly_detector.rebuild()
                    with STATE.lock:
                        STATE.normal_anomaly_result = STATE.normal_anomaly_detector.empty_result(
                            status="collecting",
                            message="检测到机位明显变化，正在自动重新采集正常道路基准。",
                        )
        frames += 1
        now = time.time()
...
def normal_anomaly_loop() -> None:
    """道路异常独立线程。

    - 只消费 capture_loop 的最新帧，不创建第二个 VideoCapture；
    - 与 normal_lane_detector 共用 STATE.normal_roi_normalized；
    - ROI 确认后先从当前视频源实时采集正常道路多基准；
    - 基准就绪后再执行直接 ORB/Homography、多基准差分和时间确认；
    - 与 YOLO/OCR/车道线线程解耦，异常检测慢时不会阻塞视频读取。
    """
    log("Runtime road anomaly worker started.")
    ...
    while not STATE.stop_event.is_set():
        ...
        detector_key = normalize_detector_model(
            getattr(STATE.config, "detector_model", "plate")
        )
        ...
        with STATE.lock:
            ...
            frame_id = int(STATE.latest_frame_id)
            frame_ts = float(STATE.latest_frame_ts or 0.0)
            vehicle_boxes = [dict(item) for item in STATE.latest_yolo_boxes]
            roi_ready = len(STATE.normal_roi_normalized) == 4

        if frame is None or not roi_ready:
            time.sleep(0.05)
            continue

        try:
            result = STATE.normal_anomaly_detector.process(
                frame=frame,
                frame_id=frame_id,
            ...
            with STATE.lock:
                STATE.normal_anomaly_result = result
                # yolo_loop 会周期性整体刷新 latest_result；这里同步补丁，
                # 让前端不必等到下一次 YOLO 才看到采集进度或异常框。
                STATE.latest_result["road_anomaly"] = result
                analysis = STATE.latest_result.get("normal_road_analysis")
                if isinstance(analysis, dict):
                    analysis["baseline_status"] = str(result.get("status", ""))
                    analysis["baseline_count"] = int(result.get("baseline_count", 0) or 0)
                    analysis["baseline_target"] = int(result.get("baseline_target", 0) or 0)
                    analysis["baseline_progress"] = int(result.get("baseline_progress", 0) or 0)
                    analysis["baseline_ready"] = bool(result.get("baseline_ready", False))
                    analysis["anomaly_candidate_count"] = int(result.get("candidate_count", 0) or 0)
                    analysis["anomaly_confirmed_count"] = int(result.get("confirmed_count", 0) or 0)
                    analysis["anomaly_total_events"] = int(result.get("total_confirmed_events", 0) or 0)
                    analysis["anomaly_processing_ms"] = float(result.get("processing_ms", 0.0) or 0.0)
                    analysis["message"] = str(result.get("message") or analysis.get("message") or "")

            STATE.update_status(
                normal_baseline_status=str(result.get("status", "")),
                normal_baseline_count=int(result.get("baseline_count", 0) or 0),
                normal_baseline_target=int(result.get("baseline_target", 0) or 0),
                normal_baseline_progress=int(result.get("baseline_progress", 0) or 0),
                normal_anomaly_candidates=int(result.get("candidate_count", 0) or 0),
                normal_anomaly_confirmed=int(result.get("confirmed_count", 0) or 0),
                normal_anomaly_total_events=int(result.get("total_confirmed_events", 0) or 0),
                normal_anomaly_ms=float(result.get("processing_ms", 0.0) or 0.0),
            )
            perf_add(
                "normal_anomaly_ms",
                float(result.get("processing_ms", 0.0) or 0.0),
            )
        except Exception as anomaly_exc:
            log(f"Runtime road anomaly error: {anomaly_exc}")
            log(traceback.format_exc())
            with STATE.lock:
                STATE.normal_anomaly_result = (
                    STATE.normal_anomaly_detector.empty_result(
                        frame_id=frame_id,
                        frame_width=int(frame.shape[1]),
                        frame_height=int(frame.shape[0]),
                        status="error",
                        message=f"道路异常检测失败：{anomaly_exc}",
                    )
                )
                STATE.latest_result["road_anomaly"] = STATE.normal_anomaly_result
            STATE.update_status(
                normal_baseline_status="error",
                message=f"道路异常检测失败：{anomaly_exc}",
            )
            time.sleep(0.15)
    ...
    while not STATE.stop_event.is_set():
        ...
        try:
            ...
            for det in detections:
                ...
                use_ocr_for_this = bool(
                    (is_plate_model or sem_type == "plate_number")
                    and ocr_service_ready()
                )
            ...
            with STATE.lock:
                normal_anomaly_info = dict(STATE.normal_anomaly_result)

            if detector_key != "normal":
                normal_anomaly_info = STATE.normal_anomaly_detector.empty_result(
                    frame_id=fid,
                    frame_width=w,
                    frame_height=h,
                    status="disabled",
                    message="当前不是 normal.onnx 模式，道路异常检测未运行。",
                )

            baseline_status = str(normal_anomaly_info.get("status", "waiting_roi") or "waiting_roi")
            if detector_key != "normal":
                normal_message = "当前模式未启用正常道路分析。"
            elif baseline_status == "collecting":
                normal_message = str(
                    normal_anomaly_info.get("message")
                    or "正在从当前视频源采集正常道路基准。"
                )
                ...
                normal_message = str(
                    normal_anomaly_info.get("message")
                    or (
                        f"ROI 内识别到 {len(normal_vehicle_boxes)} 辆正常车辆，"
                        f"已绘制 {int(normal_lane_info.get('stable_lane_count', 0) or 0)} 条稳定车道线。"
                    )
                )
            else:
                normal_message = str(
            ...
            normal_road_analysis = {
                ...
                "baseline_count": int(normal_anomaly_info.get("baseline_count", 0) or 0),
                "baseline_target": int(normal_anomaly_info.get("baseline_target", 0) or 0),
                "baseline_progress": int(normal_anomaly_info.get("baseline_progress", 0) or 0),
                "baseline_ready": bool(normal_anomaly_info.get("baseline_ready", False)),
                "anomaly_candidate_count": int(normal_anomaly_info.get("candidate_count", 0) or 0),
                "anomaly_confirmed_count": int(normal_anomaly_info.get("confirmed_count", 0) or 0),
                "anomaly_total_events": int(normal_anomaly_info.get("total_confirmed_events", 0) or 0),
                "anomaly_processing_ms": float(normal_anomaly_info.get("processing_ms", 0.0) or 0.0),
            ...
            with STATE.lock:
                ...
                STATE.latest_result = {
                    ...
                    "road_anomaly": normal_anomaly_info,
                    "road_map": road_map,
            ...
            STATE.update_status(
                ...
                normal_vehicle_count=len(normal_vehicle_boxes),
                normal_baseline_status=str(normal_anomaly_info.get("status", "")),
                normal_baseline_count=int(normal_anomaly_info.get("baseline_count", 0) or 0),
                normal_baseline_target=int(normal_anomaly_info.get("baseline_target", 0) or 0),
                normal_baseline_progress=int(normal_anomaly_info.get("baseline_progress", 0) or 0),
                normal_anomaly_candidates=int(normal_anomaly_info.get("candidate_count", 0) or 0),
                normal_anomaly_confirmed=int(normal_anomaly_info.get("confirmed_count", 0) or 0),
                normal_anomaly_total_events=int(normal_anomaly_info.get("total_confirmed_events", 0) or 0),
                normal_anomaly_ms=float(normal_anomaly_info.get("processing_ms", 0.0) or 0.0),
...
def clear_runtime_state() -> None:
    with STATE.lock:
        ...
        STATE.normal_lane_result = STATE.normal_lane_detector.empty_result(
            status="waiting_roi",
            message="正常道路模式需要先选择四点 ROI。",
        )
        STATE.normal_anomaly_detector.reset(
            message="正常道路模式需要先选择四点 ROI。"
        )
        STATE.normal_anomaly_result = STATE.normal_anomaly_detector.empty_result(
            status="waiting_roi",
        ...
        STATE.status["normal_baseline_status"] = "waiting_roi"
        STATE.status["normal_baseline_count"] = 0
        STATE.status["normal_baseline_progress"] = 0
        STATE.status["normal_anomaly_candidates"] = 0
        STATE.status["normal_anomaly_confirmed"] = 0
        STATE.status["normal_anomaly_total_events"] = 0
        STATE.status["normal_anomaly_ms"] = 0.0
        STATE.parking_tracks.clear()
        STATE.parking_zones.clear()
        STATE.status["parking_alerts"] = 0
        STATE.status["parking_active"] = 0
...
def stop_current_worker() -> None:
    ...
    for th in [
        STATE.capture_thread,
        STATE.yolo_thread,
        STATE.ocr_thread,
        STATE.normal_anomaly_thread,
    ]:
    ...
    STATE.normal_anomaly_thread = None
...
def start_pipeline(
    ...
    # 视频工作流的硬依赖只有“当前模式对应的 ONNX Session”。
    # 前端切换模型与点击启动可能间隔很短，因此最多等待 15 秒，
    # 避免异步模型切换尚未完成时直接返回“模型未就绪”。
    load_deadline = time.time() + 15.0
    requested_once = False
    while time.time() < load_deadline:
        loaded_detector = normalize_detector_model(
            STATE.status.get("detector_model", "")
        )
        if STATE.ort_session is not None and loaded_detector == detector_key:
            break

        if not STATE.model_switch_event.is_set() and not requested_once:
            requested_once = True
            load_models(allow_ocr=False)
    ...
    if STATE.ort_session is None or loaded_detector != detector_key:
        raise HTTPException(
            status_code=500,
            detail=(
                f"{detector_key} ONNX 模型未就绪："
                f"{STATE.status.get('message')}"
            ),
        )

    # 只有车牌工作流需要 OCR。OCR 在独立 GPU 子进程中加载，
    # 不重新创建 ONNX Session，也不会触碰当前视频连接。
    if detector_key == "plate" and not ocr_service_ready():
    ...
    if detector_key == "normal":
        ...
        if len(cleaned_normal_roi) != 4:
            raise HTTPException(
                status_code=400,
                detail="道路异常模式必须先在当前视频预览中选择严格 4 个 ROI 顶点，再点击开始检测。",
            )
    ...
    if detector_key == "normal":
        ...
        STATE.normal_lane_result = STATE.normal_lane_detector.empty_result(
            status="waiting",
            message="四点 ROI 已确认，等待第一帧进行连续车道线检测。",
        )
        if not STATE.normal_anomaly_detector.configure(
            cleaned_normal_roi,
            source=source,
            ...
            raise HTTPException(
                status_code=400,
                detail="道路异常实时基准采集初始化失败，请重新选择四点 ROI。",
            )
        STATE.normal_anomaly_result = STATE.normal_anomaly_detector.empty_result(
            status="collecting",
            message="四点 ROI 已确认，启动后将从当前视频源实时采集正常道路基准。",
        )

    sync_mode = "手持防拖影" if source_type == "camera" else "固定/本地视频稳定显示"
    normal_note = (
        "；四点 ROI、连续车道线和实时正常道路基准采集已启用"
        if detector_key == "normal"
        else ""
    )
    STATE.update_status(
    ...
    # OCR 线程只属于车牌模式且模型成功初始化时才启动。
    if detector_key == "plate" and ocr_service_ready():
        STATE.ocr_thread = threading.Thread(target=ocr_loop, daemon=True)

    # 实时正常基准和异常检测线程只属于 normal 模式。
    if detector_key == "normal":
        STATE.normal_anomaly_thread = threading.Thread(
            target=normal_anomaly_loop,
            daemon=True,
        )

    STATE.capture_thread.start()
    STATE.yolo_thread.start()
    if STATE.ocr_thread is not None:
        STATE.ocr_thread.start()
    if STATE.normal_anomaly_thread is not None:
        STATE.normal_anomaly_thread.start()


@app.on_event("startup")
def on_startup() -> None:
    init_system_database()
    # ONNX 与 PaddleOCR 分属两个进程，可以并行预加载。
    # OCR 子进程失败不会阻塞 normal / vehicle / stop 模式。
    threading.Thread(
        target=load_models,
        kwargs={"allow_ocr": False},
        daemon=True,
        name="onnx-model-loader",
    ).start()
    threading.Thread(
        target=ensure_paddleocr_gpu_subprocess,
        daemon=True,
        name="paddle-ocr-process-loader",
    ).start()
...
def on_shutdown() -> None:
    try:
        ...
        pass
    stop_paddleocr_gpu_subprocess()


atexit.register(stop_paddleocr_gpu_subprocess)
...
def api_ocr_restart() -> Dict[str, Any]:
    stop_paddleocr_gpu_subprocess()
    ok = ensure_paddleocr_gpu_subprocess()
    return {
        "ok": ok,
        "ocr_ready": ocr_service_ready(),
        "ocr_error": STATE.status.get("ocr_error", ""),
        "ocr_runtime": STATE.status.get("ocr_runtime", ""),
        "ocr_process_pid": STATE.status.get("ocr_process_pid", 0),
    }
...
def api_normal_roi_preview(req: NormalRoiPreviewRequest) -> Dict[str, Any]:
    ...
    return {
        ...
        "message": "已复用当前视频帧，请沿道路边界依次点击严格 4 个点。" if reused_live_frame else "请沿正常道路边界依次点击严格 4 个点。",
    }
...
def api_normal_roi_configure(req: NormalRoiConfigureRequest) -> Dict[str, Any]:
    ...
    if len(cleaned) != 4:
        raise HTTPException(status_code=400, detail="道路异常 ROI 必须严格选择 4 个有效顶点。")

    with STATE.lock:
        current_source = str(STATE.status.get("source", ""))
        current_source_type = str(STATE.status.get("source_type", "idle"))
        STATE.normal_roi_normalized = cleaned
        STATE.normal_lane_detector.configure(cleaned)
        STATE.normal_lane_result = STATE.normal_lane_detector.empty_result(
            status="waiting",
            message="四点道路 ROI 已更新，等待下一帧绘制连续车道线。",
        )
        if not STATE.normal_anomaly_detector.configure(
            cleaned,
            ...
            raise HTTPException(status_code=400, detail="实时正常道路基准采集初始化失败。")
        STATE.normal_anomaly_result = STATE.normal_anomaly_detector.empty_result(
            status="collecting",
            message="四点 ROI 已更新，正在从当前视频源重新采集正常道路基准。",
        )
        STATE.status["normal_roi_ready"] = True
        STATE.status["normal_baseline_status"] = "collecting"
        STATE.status["normal_baseline_count"] = 0
        STATE.status["normal_baseline_progress"] = 0
        STATE.status["normal_anomaly_candidates"] = 0
        STATE.status["normal_anomaly_confirmed"] = 0

    reset_visual_state("四点道路 ROI 已更新；视频连接保持不变，开始实时采集正常道路基准。")
    return {
        "ok": True,
        "points": cleaned,
        "running": bool(STATE.status.get("running")),
        ...
        "message": "已开始从当前视频源采集正常道路基准。",
    }


@app.post("/api/normal/baseline/rebuild")
def api_normal_baseline_rebuild(req: NormalBaselineRebuildRequest) -> Dict[str, Any]:
    """不重连视频源，使用当前四点 ROI 重新采集正常道路基准。"""
    detector_key = normalize_detector_model(getattr(STATE.config, "detector_model", "plate"))
    if detector_key != "normal":
        raise HTTPException(status_code=400, detail="请先切换到道路异常 normal.onnx 模式。")

    with STATE.lock:
        roi_ready = len(STATE.normal_roi_normalized) == 4
        running = bool(STATE.status.get("running"))
    if not roi_ready:
        raise HTTPException(status_code=400, detail="请先选择严格四点道路 ROI。")
    if not running:
        raise HTTPException(status_code=400, detail="请先启动本地视频或手机视频流。")

    if not STATE.normal_anomaly_detector.rebuild():
        raise HTTPException(status_code=500, detail="重新采集正常道路基准失败。")
    ...
    return {
        "ok": True,
        "stream_preserved": True,
        "message": "已清空旧基准，正在从当前视频源重新采集正常道路数据。",
    }
...
def api_normal_debug_png() -> Response:
    """返回道路异常辅助视图：蓝色差分底图、红色异常投票区域。"""
    data, frame_id = STATE.normal_anomaly_detector.get_debug_preview_png()
    if not data:
        placeholder = np.zeros((270, 480, 3), dtype=np.uint8)
        placeholder[:] = (8, 14, 24)
        cv2.putText(
            placeholder,
            "Waiting for road baseline / anomaly frame...",
            (24, 138),
    ...
    return Response(
        ...
        headers={
            ...
            "Pragma": "no-cache",
            "X-Road-Debug-Frame": str(frame_id),
        },
    )
